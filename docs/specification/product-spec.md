...
- доступ в глобальный операторский контур в MVP лучше задавать через system-level allowlist аутентифицированных external subjects, а не через обычные роли workspace;
...
Для первой рабочей поставки стоит разделять два вопроса: протокол и конкретный provider.
...
- для первой внешней версии использовать GitHub OAuth как самый быстрый путь для инженерной аудитории, если основная команда уже живет в GitHub;
...
Для архитектурной документации проекта следует использовать подход C4.

На текущем этапе достаточно поддерживать:

- system context view;
- container view;
- далее, по мере стабилизации модульных границ, component view для API;
- дополнительные deployment и dynamic views для локального профиля, auth и asset upload.
...
- Достаточно ли в MVP owner-only admin-контура, или рано понадобится отдельная роль оператора между `owner` и `editor`?
- Достаточно ли в MVP system-level allowlist операторов, или понадобится отдельная постоянная модель `PlatformOperator` в данных?
- Подтвердить ли GitHub OAuth как стартовый provider для MVP с сохранением пути на переход к полноценному OIDC-провайдеру позже?
- Достаточно ли для MVP прямых agent credentials, или нужен exchange-механизм в короткоживущие access tokens?
...
- Достаточно ли одной родительской группы для задачи, или позже потребуется поддержка множественного включения?
- Должны ли зависимости между задачами быть только типа `blocks`, или в будущем понадобятся и другие типы связей выполнения?
